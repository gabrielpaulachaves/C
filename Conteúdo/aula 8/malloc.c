...
int main(void){
       ...
       //o ponteiro ai nessa funcao está servindo apenas para apontar onde está o endereço e ajudando a colocar, baseado no endereço que el guarda.
                                //aqui estamos calculando o tamanho
       if(ponteiro != NULL){
        int i;
        //aqui, basicamente esse for é para colocar valores nos espaós reservados na memoria que estao guardados nesse ponteiro
        for(i=0;i<5;i++){
            ponteiro[i]= i+1;
            printf("%d\n", ponteiro[i]);
        };
